def counting_sort(arr):
    ...
    count_arr = [0] * (k + 1) # O(k). Can say O(n)
    ...
    for i in range(1, k + 1): # Order O(k). Can say # Order O(n)
...
of input array (O(n)). Afterwards we compute a cummulative sum over the count array, 
